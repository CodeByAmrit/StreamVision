  ...
  constructor() {
    ...
    this.promAuth =
      process.env.PROMETHEUS_USER && process.env.PROMETHEUS_PASS
        ? "Basic " +
          Buffer.from(`${process.env.PROMETHEUS_USER}:${process.env.PROMETHEUS_PASS}`).toString(
            "base64"
          )
        : null;
    this.lokiAuth =
      process.env.LOKI_USER && process.env.LOKI_PASS
        ? "Basic " +
          Buffer.from(`${process.env.LOKI_USER}:${process.env.LOKI_PASS}`).toString("base64")
        : null;
  }
  ...
  async asyncGetStructuredReport(timeframe = "now-30d") {
    ...
    const [cpuRes, ramRes, reqRes, err5xxRes, logsSummary] = await Promise.all([
      this.queryPrometheus(
        `avg_over_time((100 - (avg by(instance) (irate(node_cpu_seconds_total{mode="idle"}[5m])) * 100))[${duration}:1h])`
      ),
      this.queryPrometheus(
        `avg_over_time(((node_memory_MemTotal_bytes - node_memory_MemAvailable_bytes) / node_memory_MemTotal_bytes * 100)[${duration}:1h])`
      ),
      this.queryPrometheus(
        `sum(increase(traefik_entrypoint_requests_total{entrypoint="websecure"}[${duration}]))`
      ),
      this.queryPrometheus(
        `sum(increase(traefik_entrypoint_requests_total{entrypoint="websecure", code=~"5.."}[${duration}]))`
      ),
      ...
      services: [
        {
          ...
          status: Number(errorRate) < 1 ? "healthy" : "warning",
          details: `${totalRequests.toLocaleString()} secure connections routed with ${(100 - errorRate).toFixed(2)}% success.`,
        },
        {
          name: "Platform Management Service",
          status: avgCpu < 70 ? "healthy" : "warning",
          details: `Core system responsive with ${avgCpu.toFixed(1)}% avg processing load.`,
        },
        {
          name: "Live Processing Engine",
          status: "healthy",
          details: "Video streaming pipelines operational. Hardware throughput nominal.",
        },
      ],
      alerts: [],
    };
